...
def face_detect(img_path, encoding_path = "encoding", pre_tolerance = 0.2, tolerance = 0.6):
    ...
    for compare_encoding_dict in encoding_dict_list:
        ...
        if  abs(compare_encoding[0] - target_img_encoding[0]) < pre_tolerance :
    ...
    print(encoding_id_list)
    sorted(encoding_list, key=lambda dic: dic['dist'])
    #dis_list = fr.face_distance(encoding_list, target_img_encoding)
    print(encoding_list)
    ans = encoding_list[0]
    if ans['dist'] < tolerance:
        #return encoding_id_list[ans]
        return [encoding_id for encoding_id in encoding_list['id']]
    else:
        return -1
...
if __name__ == "__main__":
    ...
    #file_id, path= face_save("20200419-211356.jpg")
    #print(file_id, path)
    #print(face_detect("20200419_205527.jpg"))
    
    #tmp_img_list = glob.glob("*.jpg")
    #for img_path in tmp_img_list:
    #    print( img_path, face_count(img_path))
    #    print( face_save(img_path))
    #img = fr.load_image_file(img_path)
    #print(img_path)
    #print(face_count(img))

    face_webcam_save(img_cnt= 20)
